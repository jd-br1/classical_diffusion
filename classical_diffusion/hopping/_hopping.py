...
def get_ensemble_probabilities[L: Lattice](
    ...
    solver: Callable = _get_jensen_probabilities_jit,
    ...
    # Optimal number of terms for Jensen solver - needs including in JIT function
    total_outgoing_rates = jnp.sum(hop_rates, axis=-1)
    gamma = jnp.max(total_outgoing_rates) * 1.05
    lam = gamma * times[-1]
    print(lam + 2 * jnp.sqrt(lam) + 10)

    sol = solver(initial_p, times, hop_sites, hop_rates)
